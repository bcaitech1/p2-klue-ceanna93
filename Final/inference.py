from transformers import XLMRobertaTokenizer, XLMRobertaForSequenceClassification, Trainer, TrainingArguments, XLMRobertaConfig, ElectraTokenizer, ElectraForSequenceClassification, BertForSequenceClassification, BertModel, BertTokenizer
...
def ensemble_inference(xml_model, hyper_model, koelec_model, bert_model, tokenzied_sent_set, device):
  ...
  for i, data in enumerate(dataloader):
    ...
    bert_logits = bert_pred[0].mul(0.70)
...
def load_test_dataset(dataset_dir, tokenizer):
  ...
  # tokenizing dataset
  tokenized_test = bert_tokenized_dataset(test_dataset, tokenizer)
  return tokenized_test, test_label

def load_ensemble_test_dataset(dataset_dir, xml_tokenizer, hy_tokenizer, ko_tokenizer, bert_tokenizer):
  test_dataset = load_data(dataset_dir)
  test_label = test_dataset['label'].values
  # tokenizing dataset
  ...
  bert_tokenized_test = bert_tokenized_dataset(test_dataset, bert_tokenizer)
  return xml_tokenized_test, hyper_tokenized_test, koelec_tokenized_test, kobert_tokenized_test, test_label
...
def main(args):
  ...
  if args.is_ensemble == True:
    ...
    kobert_tokenizer = BertTokenizer.from_pretrained('bert-base-multilingual-cased')
    ...
    bert_model = BertForSequenceClassification.from_pretrained(args.bert_model_dir).to(device)

    test_dataset_dir = "/opt/ml/input/data/test/test.tsv"
    xml_dataset, hyper_dataset, koelec_dataset, bert_dataset, test_label = load_ensemble_test_dataset(test_dataset_dir, xml_tokenizer, hyper_tokenizer, koelec_tokenizer, kobert_tokenizer)
    test_dataset = ENSEMBLE_Dataset(xml_dataset, hyper_dataset, elec_dataset, bert_dataset, test_label)

    pred_answer = ensemble_inference(xml_model, hyper_model, koelec_model, bert_model, test_dataset, device)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--bert_model_dir', type=str, default="./results/expr/checkpoint-2500")
